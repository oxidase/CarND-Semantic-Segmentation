        ...
        for step, (images, labels) in enumerate(get_batches_fn(batch_size)):
            _, loss = sess.run([train_op, cross_entropy_loss],
                               feed_dict={input_image: images,
                                          ...
                                          keep_prob: 1,
                                          learning_rate: 0.001})
            print('Epoch {} of {} epochs, step {}, loss = {}'.format(epoch, epochs, step, loss))
            sys.stdout.flush()
        print ('Epoch training {} took {} minutes'.format(epoch, (time.time() - t) / 60))
...
def run():
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
        ...
        tf.train.write_graph(sess.graph_def, 'model', 'train.pbtxt')
